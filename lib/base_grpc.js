...
module.exports = class BaseGrpc {
  ...
  constructor(ctx, ProtoClass) {
    ...
    this.name = ProtoClass.paths.join('.');
  ...
  _getRpcClient(rpc) {
    ...
    if (!this[CLIENT]) {
      const serviceEndpoint = this.config.endpoints && this.config.endpoints[this.name];
      ...
      this[CLIENT] = new this.ProtoClass(serviceEndpoint || this.config.endpoint, grpc.credentials.createInsecure(), Object.assign({}, this.config.clientOpts));
    }
    // TODO: config.ssl
    return this[CLIENT];
  }
