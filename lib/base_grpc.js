...
module.exports = class BaseGrpc {
  ...
  _beforeRequest(invokeArgs) {
    ...
    // `x-real-ip` and `x-real-port` can not be covered
    const ip = this.ctx.get('x-real-ip') || this.ctx.ip || '';
    const port = this.ctx.get('x-real-port') || this.ctx.request.socket.remotePort || '';
    invokeArgs.metadata.set('x-real-ip', ip.toString());
    invokeArgs.metadata.set('x-real-port', port.toString());
